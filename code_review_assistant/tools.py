...
async def analyze_code_structure(code: str, tool_context: ToolContext) -> dict[str, Any]:
    ...
    try:
        ...
        if not code or not isinstance(code, str):
            return {"status": "error", "message": "No code provided or invalid input"}
        ...
        with ThreadPoolExecutor() as executor:
            ...
            analysis = await loop.run_in_executor(executor, _extract_code_structure, tree, code)
        ...
        logger.info(
            f"Tool: Analysis complete - {analysis['metrics']['function_count']} functions, "
        ...
        return {"status": "error", "error_type": "syntax", "message": error_msg, "line": e.lineno, "offset": e.offset}
    except Exception as e:
        error_msg = f"Analysis failed: {str(e)}"
        logger.error(f"Tool: {error_msg}", exc_info=True)

        return {"status": "error", "error_type": "parse", "message": error_msg}
...
async def check_code_style(code: str, tool_context: ToolContext) -> Dict[str, Any]:
    ...
    try:
        ...
        if not code:
            ...
            if not code:
                return {"status": "error", "message": "No code provided or found in state"}

        # Run style check in thread pool
        loop = asyncio.get_event_loop()
        with ThreadPoolExecutor() as executor:
            result = await loop.run_in_executor(executor, _perform_style_check, code)
        ...
        logger.info(f"Tool: Style check complete - Score: {result['score']}/100, Issues: {result['issue_count']}")
        return result
        ...
        return {"status": "error", "message": error_msg, "score": 0}
...
def _perform_style_check(code: str) -> Dict[str, Any]:
    ...
    try:
        ...
        for line in output.strip().split("\n"):
            ...
            parts = line.rsplit(":", 3)
            if len(parts) == 4:
                file_path, line_no, col_no, remainder = parts
                message_parts = remainder.strip().split(maxsplit=1)
                code = message_parts[0] if message_parts else "E000"
...
async def search_past_feedback(developer_id: str, tool_context: ToolContext) -> Dict[str, Any]:
    ...
    try:
        ...
        if hasattr(tool_context, "search_memory"):
            try:
                # Perform structured searches with timeout
                queries = [
                ...
                all_feedback = []
                patterns = {"common_issues": [], "improvements": [], "strengths": []}

                # Add timeout to prevent hanging (5 seconds per query)
                for query in queries:
                    try:
                        ...
                        logger.warning(f"Tool: Memory search timeout for query: {query}")
                        # Continue with next query instead of failing completely
                        continue
                    except Exception as query_error:
                        logger.warning(f"Tool: Memory search error for query {query}: {query_error}")
                ...
                return {
                    "status": "success",
                    "feedback_found": len(all_feedback) > 0,
        ...
        tool_context.state[StateKeys.PAST_FEEDBACK] = []
        return {"status": "error", "message": error_msg, "feedback_found": False}
...
async def update_grading_progress(tool_context: ToolContext) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.info(f"Tool: Progress updated - Attempt #{attempts}, Lifetime: {lifetime_submissions}")
        return {
            "status": "success",
...
async def validate_fixed_style(tool_context: ToolContext) -> Dict[str, Any]:
    ...
    try:
        ...
        if not code_fixes:
            return {"status": "error", "message": "No fixed code found in state"}
        ...
        loop = asyncio.get_event_loop()
        with ThreadPoolExecutor() as executor:
            style_result = await loop.run_in_executor(executor, _perform_style_check, code_fixes)
        ...
        logger.info(f"Tool: Fixed code style score: {style_result['score']}/100 (improvement: +{improvement})")
        return {
            "status": "success",
...
async def compile_fix_report(tool_context: ToolContext) -> Dict[str, Any]:
    ...
    try:
        ...
        report = {
            ...
            "improvements": {"tests": test_improvement, "style": style_improvement},
        ...
        return {"status": "success", "fix_status": fix_status, "report": report}
...
def exit_fix_loop(tool_context: ToolContext) -> Dict[str, Any]:
    ...
    return {"status": "success", "message": "Fix complete, exiting loop"}
...
async def save_fix_report(tool_context: ToolContext) -> Dict[str, Any]:
    ...
    try:
        ...
        if not fix_report:
            return {"status": "error", "message": "No fix report found in state"}
        ...
        if hasattr(tool_context, "save_artifact"):
            try:
                ...
                return {"status": "success", "filename": filename, "version": str(version), "size": len(report_json)}
            except Exception as e:
        ...
        return {"status": "success", "message": "Fix report saved to state", "size": len(report_json)}
