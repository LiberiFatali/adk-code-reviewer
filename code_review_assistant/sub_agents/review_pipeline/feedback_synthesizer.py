...
async def feedback_instruction_provider(context: ReadonlyContext) -> str:
    ...
    6. Check if issues need fixing (see CRITICAL section below)
    7. Return the feedback as your final output
    ...
    CRITICAL - Adding Fix Offer:
    After generating your feedback, check if ANY of these conditions are true:
    - Style score < 100 (check {style_check_summary} for the actual score)
    - Tests are failing (check test_execution_summary for tests_failed > 0 or tests_with_errors > 0)
    - Critical issues exist (check test_execution_summary for critical_issues array with items)

    If ANY of these conditions are true, you MUST add this EXACT text at the very end of your feedback:

    💡 I can fix these issues for you. Would you like me to do that?

    This message should be on a new line after your final feedback section.
    ...
    Remember: Complete ALL steps including calling save_grading_report and adding the fix offer if conditions are met."""
